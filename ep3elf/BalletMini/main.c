...
static int method5(VIEW_GUI* data, GUI_MSG* msg)
{
	...
	{
		if ((k == (int)cfgKeyEnter) && (vd->pos_cur_ref != 0xFFFFFFFF))
		{
		...
		else if (k == (int)cfgKeyScrollDown)
		{
		...
		else if (k == (int)cfgKeyPageDown)
		{
			scrollDown(vd, ScreenH() - 20 - scr_shift);
			vd->pos_cur_ref = 0xFFFFFFFF;
		}
		else if (k == (int)cfgKeyHalfPageDown)
		{
			scrollDown(vd, (ScreenH() - 20 - scr_shift) / 2);
			vd->pos_cur_ref = 0xFFFFFFFF;
		}
		else if (k == (int)cfgKeyPageUp)
		{
			scrollUp(vd, ScreenH() - 20 - scr_shift);
			vd->pos_cur_ref = 0xFFFFFFFF;
		}
		else if (k == (int)cfgKeyHalfPageUp)
		{
			scrollUp(vd, (ScreenH() - 20 - scr_shift) / 2);
			vd->pos_cur_ref = 0xFFFFFFFF;
		}
		else if (k == (int)cfgKeyMenu)
		{
			STOPPED = 1;
			CreateMainMenu(vd);
		}
		else if (k == (int)cfgKeyBack)
		{
			if (STOPPED)
			{
				return 0xFE;
		...
		else if (k == (int)cfgKeySearchAgain)
		{
			if (m == LONG_PRESS)
			{
				CreateFindDialog(vd);
			...
			while(ypos <= 16384)
			{
				if (LineDown(vd))
				{
					...
					else
					{
						...
						if ((vl + 1) < (int)vd->lines_cache_size)
						{
	...
	{
		if ((k == (int)cfgKeyNewCopy) || (k == (int)cfgKeyBrowser))
		{
			rf = FindReference(vd, vd->pos_cur_ref);
			if (rf)
			{

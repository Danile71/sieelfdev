...
{
	BList* bl;
	bl = FindBListByNS(&n, IS_BACK);
	if ((!n) && (bl)) return(bl);
	bl = FindBListByNS(&n, IS_FOLDER);
	if ((!n) && (bl)) return(bl);
	bl = FindBListByNS(&n, IS_FILE);
	if ((!n) && (bl)) return(bl);
	return bl;
}
...
HEADER_DESC bookmarks_menu_header = {0, 0, 0, 0, NULL, 0, 0};
...
HEADER_DESC input_url_header = {0, 0, 0, 0, NULL, 0, 0};
...
{
	{NULL, 0, 0, 0, NULL, MENU_FLAG3, MENU_FLAG2}, //0
	{NULL, 0, 0, 0, NULL, MENU_FLAG3, MENU_FLAG2}, //1
	{NULL, 0, 0, 0, NULL, MENU_FLAG3, MENU_FLAG2}, //2
	{NULL, 0, 0, 0, NULL, MENU_FLAG3, MENU_FLAG2}, //3
	{NULL, 0, 0, 0, NULL, MENU_FLAG3, MENU_FLAG2}, //3
	{NULL, 0, 0, 0, NULL, MENU_FLAG3, MENU_FLAG2}, //3
	{NULL, 0, 0, 0, NULL, MENU_FLAG3, MENU_FLAG2}, //3
	{NULL, 0, 0, 0, NULL, MENU_FLAG3, MENU_FLAG2}  //4
};
...
int CreateMainMenu(VIEWDATA* vd)
{
	...
	if ((main_csm = (MAIN_CSM*)FindCSMbyID(maincsm_id)))
	{
		...
		mm_id = CreateMenu(0, 0, &main_menu_struct, &main_menu_header, 0, MAIN_MENU_ITEMS_N, vd, 0);
		main_csm->main_menu_id = mm_id;
	}
	return mm_id;
}

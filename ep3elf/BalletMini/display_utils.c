				...
				if (y2 >= scr_h || lcheck)
				{
					...
					if (vd->lines_cache_size)
					{
						...
						if (pageheight)
						{
							...
							unsigned int b = (ycoord + vd->pixdisp) * scrh / pageheight + scr_shift;
							unsigned int e = scrh * scrh / pageheight;
							if (e < 4) e = 4;
							if (e < scrh)
							{
#ifdef ELKA
...
int ChangeMenuSelection(VIEWDATA* vd, REFCACHE* rf, unsigned int cur_pos_ref)
{
	...
	for(int o = 0; o < n_sel; o++)
	{
		...
		vd->oms_pos++;
		if ((int)rf->value == vd->oms_pos) start = o;
...
void link_options(USR_MENU_ITEM* item)
{
	...
	if (item->type == 0)
	{
		...
		{
		case 0:
			wsprintf(item->ws, per_t, "ballet");
			break;
		case 1:
			wsprintf(item->ws, per_t, "browser");
			break;
		case 2:
			wsprintf(item->ws, per_t, "nrss");
			break;
		case 3:
			wsprintf(item->ws, per_t, "sieget");
			break;
		case 4:
			ascii2ws(item->ws, lgpData[LGP_Clear]);
			break;
...
void FindStringOnPage(VIEWDATA* cur_vd)
{
	...
	unsigned int i = (cur_vd->lines_cache[cur_vd->view_line]).pos + 1;
	int j = 0;
	int found = 0;

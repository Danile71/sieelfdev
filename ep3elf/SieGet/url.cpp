...
int URLFile::Read(char * file)
{
  ...
  {
    if((f=_open(file, A_ReadOnly+A_BIN, P_READ, &err))!=-1)
    {
      if (url = new char[fstat.size+1])
      {
        url[_read(f, url, fstat.size, &err)]=0;
        _close(f, &err);
        return 1;
      }
      _close(f, &err);
...
int URLFile::Write(char * file)
{
  ...
  if ((hFile=_open(file, A_WriteOnly + A_Truncate + A_Create + A_BIN, P_WRITE, &io_error))!=-1)
  {
    _write(hFile, url, strlen(url), &io_error);

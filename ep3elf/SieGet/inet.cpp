...
void Download::onHTTPData(char * data, int size) // Запись данных в файл
{
  ...
  if (hFile == -1) // Если файл не открыт, открываем его
    hFile = _open(full_file_name, A_WriteOnly + A_Create + A_Append + A_BIN, P_WRITE, &io_error); // Только запись с созданием и добавлением в конец

  _write(hFile, data, size, &io_error); // Записываем принятые данные в файл
  //if(CFG_FFLUSH) _flush(hFile, &io_error); // Точно записываем в файл)
